...
export const showToast = ({ content, type, duration, position, toastStyle }: ToastConfig) => {
  var func
  switch (type) {
    case 'error':
      func = toast.error
...
export const showApiError = (e: any) => {
  showToast({ content: e?.response?.data?.message ?? 'Thao tác không thành công', type: 'error' })
}
